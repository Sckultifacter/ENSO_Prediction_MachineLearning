"""
=============================================================
  ENSO (Niño 3.4) Forecasting with Random Forest
=============================================================
...
# ══════════════════════════════════════════════════════════════════════════════
#
#  Base predictor physics:
#    nino34     → SST anomaly in the central-eastern Pacific (ENSO core signal)
#    soi        → Atmospheric pressure difference / trade wind strength
#    olr        → Outgoing Longwave Radiation — proxy for convection & cloud activity
#    sst_india  → Indian Ocean SST — remote influence on ENSO via atmospheric bridge
#
#  Physics-informed additions:
#    nino34_rolling3 → 3-month rolling mean captures ocean thermal memory / heat content
#    enso_phase      → Discrete phase flag (-1 La Niña, 0 Neutral, +1 El Niño)
#    sst_gradient    → Indian Ocean minus Pacific SST — inter-basin coupling strength
#    sin_month /
#    cos_month       → Seasonal cycle encoding (Spring Predictability Barrier, boreal winter peak)
# ══════════════════════════════════════════════════════════════════════════════

# Original base features (lagged versions of all are created below)
_BASE_FEATURE_COLS = ["nino34", "soi", "olr", "sst_india"]

# All features passed into the lag machinery (base + physics-informed)
FEATURE_COLS = [
    "nino34", "soi", "olr", "sst_india",   # original
    "nino34_rolling3",                      # ocean thermal memory
    "enso_phase",                           # discrete ENSO state
    "sst_gradient",                         # inter-basin SST coupling
    "sin_month", "cos_month",               # seasonal cycle
]
...
def _add_physics_features(df):
    """
    Compute and attach all physics-informed columns to df (in-place safe copy).
    Called once on the clean merged dataframe before lag creation.
    """
    # 1. Ocean thermal memory: 3-month rolling mean of Niño 3.4
    df["nino34_rolling3"] = df["nino34"].rolling(3).mean()

    # 2. ENSO phase indicator: El Niño (+1), La Niña (-1), Neutral (0)
    df["enso_phase"] = 0
    df.loc[df["nino34"] >  0.5, "enso_phase"] =  1
    df.loc[df["nino34"] < -0.5, "enso_phase"] = -1

    # 3. SST gradient: Indian Ocean minus Pacific — inter-basin coupling
    df["sst_gradient"] = df["sst_india"] - df["nino34"]

    # 4. Seasonal encoding: sine/cosine of calendar month
    df["month"]     = df["time"].dt.month
    df["sin_month"] = np.sin(2 * np.pi * df["month"] / 12)
    df["cos_month"] = np.cos(2 * np.pi * df["month"] / 12)
    df.drop(columns=["month"], inplace=True)   # month itself not needed as a raw feature
...
def build_dataset(df, lead, lags=LAG_MONTHS):
    ...
    # Physics-informed features (must be added before lag creation)
    df = _add_physics_features(df)

    # Lag every feature column (base + physics-informed)
    lag_names = []
    ...
    # (rolling(3) introduces 2 NaNs at the start; shift(-lead) removes `lead` rows at the end)
...
def plot_predictions(results_dict, out_dir):
    ...
    fig.update_layout(
        title=dict(text="ENSO Niño 3.4 Forecast — Random Forest",
                   font=dict(size=20, color="white"), x=0.5),
        height=400 * n,
        **_plotly_dark_layout(),
    )

    path = f"{out_dir}/enso_predictions_phy(rf).html"
...
def plot_performance_summary(results, out_dir):
    ...
    fig.update_layout(
        title=dict(text="Forecast Skill — Random Forest",
    ...
    path = f"{out_dir}/enso_performance_phy(rf).html"
    pio.write_html(fig, path, include_plotlyjs="cdn")
...
def plot_error_timeseries(results_dict, out_dir):
    ...
    fig.update_layout(
        title=dict(text="Forecast Error Over Time by Lead — Random Forest",
                   font=dict(size=18, color="white"), x=0.5),
        xaxis_title="Time",
    ...
    path = f"{out_dir}/enso_error_timeseries_phy(rf).html"
    pio.write_html(fig, path, include_plotlyjs="cdn")
...
def run_pipeline(nino_path, soi_path, olr_path, sst_path,
    ...
    print("Training Random Forest models(using physics)\n")
    for lead in lead_times:
        ...
        if lead >= 9:
            print(f"  [Lead {lead}m] Low skill expected due to ENSO physical limits "
                  f"(Spring Predictability Barrier)")
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(BASE_DIR, "outputs", "rf")
